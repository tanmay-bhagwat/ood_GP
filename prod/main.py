...
model = GPModel(log_noise=-6.).to(device=device, dtype=torch.float64)
kernel = StructKernel(D=train_X_norm.shape[-1], learnable=learnable, separate_ll=False).to(device=device, dtype=torch.float64)
